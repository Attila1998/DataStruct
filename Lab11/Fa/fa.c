...
//gyökér - bal - jobb
    ...
    {
        printf("%d",tree->value);//gyoker
        Preorder(tree->left);      //bal
        Preorder(tree->right);     //jobb
    }
}
...
//bal - jobb - gyökér
    if(tree)
    {
        Postorder(tree->left);      //bal
        Postorder(tree->right);     //jobb
        printf("%d",tree->value);//gyoker
    }
}
...
//ha a törlendo elem nagyobb mint a gyökér elem
    ...
    else
    {
        ...
        //törlés
        root->right = Delete(tree->right;temp->value);
    }
    return tree;
}
