#ifndef LANCOLTLISTA_H_INCLUDED
#define LANCOLTLISTA_H_INCLUDED

typedef struct PELEM{
    int ata; //adatmezõ
    struct PELEM *next; //következõ elem címe, ha nincs, akkor 0.
}PELEM;

//egy elemre vonatkozó függvények
PELEM* Create(int); //létrehoz egy elemet
void Destroy(PELEM*); //felszabadít egy elemet
//lista
PELEM* CreateL(); //return NULL;
void DestroyL(PELEM*); //felszabadítja a teljes listát
PELEM* insertLast(PELEM*, int); //beszúr a lista utolsó eleme után
PELEM* insertFirst(PELEM*, int); //beszúr a lista elsõ eleme elé
PELEM* insertOrdered(PELEM*, int); //beszúr rendezetten egy feltételezhetõen már rendezett listába
void Print(PELEM*); //kiírja a lista adat-mezõit
PELEM* SortL(PELEM*); //elrendezi a listaelemeket adatmezõ szerint növekvõ sorrendbe
PELEM* Delete(PELEM*, int); //töröl adott értékû elemet a listából

#endif // LANCOLTLISTA_H_INCLUDED
